...
class Trainer:
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            pred_left, pred_right = self.netG(updated_frames, pred_flows_bi, l_t, original_frames=original_frames)
            ...
            # comp_imgs = pred_right  # TODO: confirm
            ...
            # generator l1 loss
            whole_loss = self.l1_loss(pred_right, frames_right)
            ...
            # perceptual loss
            if self.config['losses']['perceptual_weight'] > 0:
                perc_loss = self.perc_loss(pred_right.view(-1,3,h,w), frames_right.view(-1,3,h,w))[0] * self.config['losses']['perceptual_weight']
                gen_loss += perc_loss
            ...
            if not self.config['model']['no_dis']:
                ...
                gen_clip = self.netD(pred_right_frames)
                gen_clip_l = self.netD(pred_left_frames)
                gan_loss = (self.adversarial_loss(gen_clip, True, False) + self.adversarial_loss(gen_clip_l, True, False)) * .5
            ...
            if not self.config['model']['no_dis']:
                ...
                fake_clip = self.netD(pred_right_frames.detach())
                fake_clip_l = self.netD(pred_left_frames.detach())
                dis_real_loss = (self.adversarial_loss(real_clip, True, True) + self.adversarial_loss(real_clip_l, True, True)) / 2
                dis_fake_loss = (self.adversarial_loss(fake_clip, False, True) + self.adversarial_loss(fake_clip_l, False, True)) / 2
