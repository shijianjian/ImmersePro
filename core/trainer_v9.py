...
class Trainer:
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            output = self.netG(updated_frames, pred_flows_bi, l_t, original_frames=original_frames)
            if isinstance(output, (list, tuple)) and len(output) == 4:
                pred_conversion, pred_texture, pred_texture_mask, pred_imgs = output
                pred_conversion = pred_conversion.view(b, -1, c, h, w)
                pred_texture = pred_texture.view(b, -1, c, h, w)
                pred_texture_mask = pred_texture_mask.view(b, -1, 3, h, w)
                pred_imgs = pred_imgs.view(b, -1, c, h, w)
                selection_map = None
            elif isinstance(output, (list, tuple)) and len(output) == 5:
                pred_conversion, pred_texture, pred_texture_mask, pred_imgs, selection_map = output
                pred_conversion = pred_conversion.view(b, -1, c, h, w)
                pred_imgs = pred_imgs.view(b, -1, c, h, w)
                pred_texture = pred_texture.view(b, -1, c, h, w)
                pred_texture_mask = pred_texture_mask.view(b, -1, 3, h, w)
            ...
            pred_local_texture_mask_frames = pred_texture_mask[:, :l_t, ...]
            # comp_local_frames = gt_local_frames * (1. - local_masks) +  pred_local_frames * local_masks
            ...
            def compute_diffloss(pred_imgs, gt_images):
                pred_gray = torch.mean(pred_imgs, dim=2, keepdim=True)
                gt_gray = torch.mean(gt_images, dim=2, keepdim=True)
            ...
            conversion_loss = self.l1_loss(pred_conversion * 100, frames_right * 100)
            texture_loss = self.l1_loss(pred_texture * 100, frames_right * 100) * .9 + self.l1_loss(pred_texture_mask * 100, frames_right * 100) * .1
            # hole_loss = self.l1_loss(pred_imgs * 100, frames_right * 100) * .1
            if torch.isnan(whole_loss) or torch.isnan(conversion_loss):
                continue  # TODO: Something weird may happen
            gen_loss += whole_loss
            # gen_loss += hole_loss
            gen_loss += conversion_loss
            gen_loss += texture_loss
            gen_loss += compute_diffloss(pred_conversion * 100, frames_right * 100)
            self.add_summary(self.gen_writer, 'loss/whole_loss', whole_loss.item() / 100)
            self.add_summary(self.gen_writer, 'loss/conversion_loss', conversion_loss.item() / 100)
            self.add_summary(self.gen_writer, 'loss/texture_loss', texture_loss.item() / 100)
            ...
            # perceptual loss
            if self.config['losses']['perceptual_weight'] > 0:
                perc_loss = self.perc_loss((pred_imgs.view(-1,3,h,w) + 1) / 2, (frames_right.view(-1,3,h,w) + 1 / 2))[0] * self.config['losses']['perceptual_weight']
                gen_loss += perc_loss * 100
                self.add_summary(self.gen_writer, 'loss/perc_loss', perc_loss.item())

            # gan loss
            if not self.config['model']['no_dis']:
                # generator adversarial loss
                gen_clip = self.netD(comp_imgs) / 3 + self.netD(pred_conversion) / 3
                gan_loss = self.adversarial_loss(gen_clip, True, False)
                gan_loss = gan_loss * self.config['losses']['adversarial_weight']
                gen_loss += gan_loss * 100
            ...
            if not self.config['model']['no_dis']:
                ...
                fake_clip = self.netD(comp_imgs.detach()) / 3 + self.netD(pred_conversion.detach()) / 3
                dis_real_loss = self.adversarial_loss(real_clip, True, True)
                dis_fake_loss = self.adversarial_loss(fake_clip, False, True)
                dis_loss += (dis_real_loss + dis_fake_loss) / 2 * 100
            ...
            if self.iteration % 200 == 0:
                ...
                pred_local_conversion_frames_cpu = ((pred_local_conversion_frames.view(b,-1,3,h,w) + 1)/2.0).cpu()
                pred_local_texture_frames_cpu = ((pred_local_texture_frames.view(b,-1,3,h,w) + 1)/2.0).cpu()
                pred_local_texture_mask_frames_cpu = ((pred_local_texture_mask_frames.view(b,-1,3,h,w) + 1)/2.0).cpu()
                
                if selection_map is None:
                    img_results = torch.cat([masked_local_frames[0][t], gt_local_frames_cpu[0][t], pred_local_conversion_frames_cpu[0][t],
                                             pred_local_texture_frames_cpu[0][t], pred_local_texture_mask_frames_cpu[0][t],
                                             pred_local_frames_cpu[0][t]], 1)
                else:
                    selection_map_cpu = ((selection_map.view(b,-1,1,h,w) + 1) * (255 // 100)).cpu() / 255
                    img_results = torch.cat([masked_local_frames[0][t], gt_local_frames_cpu[0][t], pred_local_conversion_frames_cpu[0][t],
                                             pred_local_texture_frames_cpu[0][t], pred_local_texture_mask_frames_cpu[0][t],
                ...
                if masked_local_frames.shape[1] > 5:
                    if selection_map is None:
                        img_results = torch.cat([masked_local_frames[0][t], gt_local_frames_cpu[0][t], pred_local_conversion_frames_cpu[0][t],
                                                 pred_local_texture_frames_cpu[0][t], 
                                                 pred_local_frames_cpu[0][t]], 1)
                    else:
                        selection_map_cpu = ((selection_map.view(b,-1,1,h,w) + 1) * (255 // 100)).cpu() / 255
                        img_results = torch.cat([masked_local_frames[0][t], gt_local_frames_cpu[0][t], pred_local_conversion_frames_cpu[0][t],
            ...
            if self.config['global_rank'] == 0:
                ...
                if not self.config['model']['no_dis']:
                    pbar.set_description((f"d: {dis_loss.item():.3f}; "
                                          f"stereo: {conversion_loss.item():.3f}; "
                                          f"stereo2: {texture_loss.item():.3f}; "
                                          f"whole: {whole_loss.item():.3f}; "))
                else:
                    pbar.set_description((f"stereo: {conversion_loss.item():.3f}; "
                                          f"stereo2: {texture_loss.item():.3f}; "
                                          f"whole: {whole_loss.item():.3f}; "))
