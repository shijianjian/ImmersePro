...
from core.dataset import TrainDatasetWithMask
...
class Trainer:
    def __init__(self, config):
        ...
        self.train_dataset = TrainDatasetWithMask(config['train_data_loader'])
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            self.iteration += 1
            frames, frames_right, masks_right, flows_f, flows_b, _ = train_data
            frames = frames.to(device)
            frames_right = frames_right.to(device)
            masks_right = masks_right.to(device)
            ...
            pred_imgs, pred_masks = self.netG(updated_frames, pred_flows_bi, l_t, original_frames=original_frames)
            pred_imgs = pred_imgs.view(b, -1, c, h, w)
            pred_masks = pred_masks.view(b, -1, 1, h, w)
            ...
            # generator l1 loss
            whole_loss = self.l1_loss(pred_imgs, frames_right)
            mask_loss = self.bce_loss(pred_masks, masks_right)
            ...
            valid_loss = self.l1_loss(pred_imgs * (1 - masks_right), frames_right * (1 - masks_right))
            # valid_loss = valid_loss / (torch.mean(1 - masks_right) * self.config['losses']['valid_weight'] + 1e-7)
            gen_loss += valid_loss
            ...
            if self.iteration % 200 == 0:
                ...
                expected_frames_cpu = ((expected_frames.view(b,-1,3,h,w) + 1)/2.0).cpu()
                gt_masked_right = (masks_right.view(b,-1,1,h,w)).cpu().repeat(1, 1, 3, 1, 1)
                pred_masks_cpu = (pred_masks.view(b,-1,1,h,w)).cpu().repeat(1, 1, 3, 1, 1)
                pred_local_frames_cpu = ((pred_local_frames.view(b,-1,3,h,w) + 1)/2.0).cpu()
                img_results = torch.cat([
                    gt_local_frames_cpu[0][t],
                    expected_frames_cpu[0][t],
            ...
            if self.config['global_rank'] == 0:
                ...
                if not self.config['model']['no_dis']:
                    pbar.set_description((f"d: {dis_loss.item():.3f}; "
                                          f"whole: {whole_loss.item():.3f}; masked: {valid_loss.item():.3f};"))
                else:
                    pbar.set_description((f"whole: {whole_loss.item():.3f}; masked: {valid_loss.item():.3f};"))
