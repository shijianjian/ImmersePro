...
class GroupRandomHorizontalFlowFlip(object):
    ...
    """
    def __call__(self, img_group, img_group_right, flowF_group, flowB_group, flowltr_group=None, masks=None):
        v = random.random()
        if v < 0.5:
            ret_img = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in img_group]
            ret_img_right = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in img_group_right]
            ret_flowF = [ff[:, ::-1] * [-1.0, 1.0] for ff in flowF_group]
            ret_flowB = [fb[:, ::-1] * [-1.0, 1.0] for fb in flowB_group]
            if flowltr_group is not None:
                ret_flowltr = [f[:, ::-1] * [-1.0, 1.0] for f in flowltr_group]
                return ret_img, ret_img_right, ret_flowF, ret_flowB, ret_flowltr
                
            if masks is not None:
                masks = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in masks]
                return ret_img, ret_img_right, ret_flowF, ret_flowB, masks
            return ret_img, ret_img_right, ret_flowF, ret_flowB
        else:
            if flowltr_group is not None:
                return img_group, img_group_right, flowF_group, flowB_group, flowltr_group
            if masks is not None:
                return img_group, img_group_right, flowF_group, flowB_group, masks
            return img_group, img_group_right, flowF_group, flowB_group


class GroupRandomHorizontalFlip(object):
    """Randomly horizontally flips the given PIL.Image with a probability of 0.5
    """
    def __call__(self, img_group, img_group_right, is_flow=False, masks=None):
        v = random.random()
        if v < 0.5:
            ret = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in img_group]
            ret_right = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in img_group_right]
            if is_flow:
            ...
            if masks is not None:
                masks = [img.transpose(Image.FLIP_LEFT_RIGHT) for img in masks]
                return ret, ret_right, masks
            return ret, ret_right
        else:
            if masks is not None:
                return img_group, img_group_right
            return img_group, img_group_right
