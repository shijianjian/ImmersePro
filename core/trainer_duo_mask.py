...
class Trainer:
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            pred_left, pred_right, masks, soft_masks, output = self.netG(updated_frames, pred_flows_bi, l_t, original_frames=original_frames)
            pred_left = pred_left.view(b, -1, c, h, w)
            pred_right = pred_right.view(b, -1, c, h, w)
            masks = masks.view(b, -1, 1, h, w)
            output = output.view(b, -1, c, h, w)

            # get the local frames
            pred_left_frames = pred_left[:, :l_t, ...]
            pred_right_frames = pred_right[:, :l_t, ...]
            local_masks = masks[:, :l_t, ...]
            local_output = output[:, :l_t, ...]
            comp_local_frames = pred_right_frames * (1. - local_masks) +  local_output * local_masks
            comp_imgs = pred_right * (1. - masks) +  output * masks
            ...
            # generator l1 loss
            whole_loss = self.l1_loss(comp_imgs, frames_right)
            recon_loss = self.l1_loss(pred_left, frames)
            hole_loss = whole_loss / torch.mean(masks) * self.config['losses']['hole_weight']
            
            gen_loss += whole_loss + hole_loss + recon_loss + self.l1_loss(frames_right * (1. - masks), pred_right * (1. - masks)) + (1 - soft_masks).mean() * 0.1
            self.add_summary(self.gen_writer, 'loss/whole_loss', whole_loss.item())
            ...
            # perceptual loss
            if self.config['losses']['perceptual_weight'] > 0:
                perc_loss = self.perc_loss(comp_imgs.view(-1,3,h,w), frames_right.view(-1,3,h,w))[0] * self.config['losses']['perceptual_weight']
                gen_loss += perc_loss
            ...
            if not self.config['model']['no_dis']:
                ...
                # gen_clip_l = self.netD(pred_left_frames)
            ...
            if not self.config['model']['no_dis']:
                ...
                # real_clip_l = self.netD(frames)
                fake_clip = self.netD(comp_imgs.detach())
                # fake_clip_l = self.netD(pred_left_frames.detach())
            ...
            if self.iteration % 100 == 0:
                ...
                prop_local_frames_cpu = ((pred_left_frames.view(b,-1,3,h,w) + 1)/2.0).cpu()
                pred_local_frames_cpu = ((pred_right_frames.view(b,-1,3,h,w) + 1)/2.0).cpu()
                masks_cpu = ((masks.view(b,-1,1,h,w))).cpu().repeat(1, 1, 3, 1, 1)
                outputs_cpu = ((output.view(b,-1,3,h,w) + 1) / 2.0).cpu()
                comp_imgs_cpu = ((comp_imgs.view(b,-1,3,h,w) + 1) / 2.0).cpu()
