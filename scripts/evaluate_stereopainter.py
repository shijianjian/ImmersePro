...
from model.stereo_painter import InpaintGenerator
...
def main_worker(args):
    ...
    assert (args.dataset == 'davis') or args.dataset == 'youtube-vos' or args.dataset == 'youtube-sbs', \
    ...
    for index, items in enumerate(test_loader):
        ...
        frames, frames_right, flows_f, flows_b, video_name, frames_PIL = items
        video_name = video_name[0]
        print('Processing:', video_name)

        video_length = frames.size(1)
        frames, frames_right = frames.to(device), frames_right.to(device)
        ...
        with torch.no_grad():
            ...
            prop_imgs = model.img_propagation(masked_frames, pred_flows_bi, 'nearest')

            b, t, _, _, _ = masked_frames.size()
            # updated_masks = updated_local_masks.view(b, t, 1, h, w)
            updated_frames = prop_imgs # merge

            del gt_flows_bi, frames
            if not args.load_flow:
                torch.cuda.empty_cache()
        ...
        for f in range(0, video_length, neighbor_stride):
            ...
            with torch.no_grad():
                ...
                pred_img = model(selected_imgs, selected_pred_flows_bi, l_t)
    ...
    if args.task == 'video_completion':
        ...
        print('Finish evaluation... Average Frame PSNR/SSIM: '
            f'{avg_frame_psnr:.2f}/{avg_frame_ssim:.4f} | Time: {avg_time:.4f}')
        eval_summary.write(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', choices=['davis', 'youtube-vos', 'youtube-sbs'], type=str)
    parser.add_argument('--video_root', default='dataset_root', type=str)
    parser.add_argument('--json_root', default=None, type=str)
    ...
    parser.add_argument('--load_flow', default=True, type=bool)
