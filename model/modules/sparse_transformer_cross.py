...
class SparseWindowAttention(nn.Module):
    ...
    def forward(self, x, disp, mask=None, T_ind=None, attn_mask=None):
        ...
        # If no mask at all.
        if mask is None:
            mask = torch.zeros((b, t, h, w, 1), device=x.device, dtype=x.dtype)

        # reverse order
        if pad_r > 0 or pad_b > 0:
            x = F.pad(x, (0, 0, 0, pad_r, 0, pad_b, 0, 0), mode='constant', value=0) 
            disp = F.pad(disp, (0, 0, 0, pad_r, 0, pad_b, 0, 0), mode='constant', value=0) 
...
class TemporalSparseTransformer(nn.Module):
    def __init__(self, dim, n_head, window_size, pool_size,
                norm_layer=nn.LayerNorm, t2t_params=None, hidden_dim=1960):
        super().__init__()
        self.window_size = window_size
        self.attention = SparseWindowAttention(dim, n_head, window_size, pool_size)
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        self.mlp = FusionFeedForward(dim, hidden_dim=hidden_dim, t2t_params=t2t_params)

    def forward(self, x, disp, fold_x_size, mask=None, T_ind=None):
        """
        Args:
        ...
        shortcut = x
        x = self.norm1(x)
        att_x = self.attention(x, disp, mask=mask, T_ind=T_ind)
...
class TemporalSparseTransformerBlock(nn.Module):
    def __init__(self, dim, n_head, window_size, pool_size, depths, t2t_params=None, hidden_dim=1960):
        super().__init__()
        blocks = []
        for i in range(depths):
             blocks.append(
                TemporalSparseTransformer(dim, n_head, window_size, pool_size, t2t_params=t2t_params, hidden_dim=hidden_dim)
             )
        self.transformer = nn.Sequential(*blocks)
        self.depths = depths

    def forward(self, x, disp, fold_x_size, l_mask=None, t_dilation=2):
        """
        Args:
            x: image tokens, shape [B T H W C]
            fold_x_size: fold feature size, shape [60 108]
        ...
        for i in range(0, self.depths):
            x = self.transformer[i](x, disp, fold_x_size, l_mask, T_ind[i])
